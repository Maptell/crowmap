...
Crowmap.MapParser = Crowmap.Base({
	...
	convert:function(){
		...
		for(var k=0;k<darray.length;k++){
			...
			var tkns=line.match(/[a-zA-Z_\-\.\\\/0-9\(\)\[\]\|\{\},]*|"[^"]+"|'[^']+'|\/[^\/]+\/'/g);
			if(!tkns)continue;
		...
		while(elm1 !='END' && this.tokens.length > 1){
			...
			if(elmc=='CONFIG'){
			   ...
			   properties['CONFIG^'+key]=val;
			    ...
			    properties['$EXTENT&minx']=minx;
			    properties['$EXTENT&miny']=miny;
			    properties['$EXTENT&maxx']=maxx;
			    properties['$EXTENT&maxy']=maxy;
	...
	parseLayer:function(pid)
	{
			   ...
			   properties['$EXTENT&minx']=minx;
                ...
                this.response.push(nmap);
                  ...
                  this.parseMetaData();
                   ...
                   var sizex=this.tokens.shift();
                  var sizey=this.tokens.shift();
                  properties['$MINFEATURESIZE&x']=sizex;
                  properties['$MINFEATURESIZE&y']=sizey;
			}else if(elmc=='NAME'){
                   var val=this.tokens.shift();
                   properties[elm1]=val;
		...
		console.log("Layeer"+JSON.stringify(properties));
		return properties;
	},
	...
	parseLegend:function(pid)
	{
		...
		do{
			...
			if(elmc=='IMAGECOLOR'){
                   ...
                   var sizex=this.tokens.shift();
                  var sizey=this.tokens.shift();
                  properties['$KEYSIZE&x']=sizex;
                  properties['$KEYSIZE&y']=sizey;
			}else if(elmc=='KEYSPACING'){
                    var sizex=this.tokens.shift();
                  var sizey=this.tokens.shift();
                  properties['$KEYSPACING&x']=sizex;
                  properties['$KEYSPACING&y']=sizey;
			}else if(elmc=='LABEL'){
                 var id=this.counter++;
	...
	parseSymbol:function()
	{
		...
		do{
			...
			if(elmc=='ANCHORPOINT'){
                  var sizex=this.tokens.shift();
                  var sizey=this.tokens.shift();
                  properties['$ANCHORPOINT&x']=sizex;
                  properties['$ANCHORPOINT&y']=sizey;
                  ...
                  console.log("There is an error parsing Symbol");break;
			}
	...
	parseColor:function()
	{
				...
				if($.isNumeric(r)){
				   ...
				   return r&&g&&b ? "#"+r.toString(16)+g.toString(16)+b.toString(16):null;
				}
				return r;
	...
	parseMetaData:function()
	{
		...
		do{
			var elm1=this.tokens.shift().toLowerCase();
			var elmc=elm1?elm1.toUpperCase():null;
			var value=this.tokens.shift().toLowerCase();
			if(elm1 && value){
				properties[elm1]=value;
			}
			...
			if(elmc=='ANGLE'){
                  ...
                  var val=this.tokens.shift();
                   ...
                   properties[elm1]=val;
                    ...
                    var sizex=this.tokens.shift();
                  var sizey=this.tokens.shift();
                  properties['$SHADOWSIZE&x']=sizex;
                  properties['$SHADOWSIZE&y']=sizey;
			}else if(elmc=='SIZE'){
                   var val=this.tokens.shift();
                   properties[elm1]=val;
			}else if(elmc=='STYLE'){
			...
			if(elmc=='COLORRANGE'){
                 ...
                 properties[elm1]= minval + " " + maxval;
                  ...
                  var val=this.tokens.shift();
                   ...
                   properties['$OFFSET&x']=x;
                   properties['$OFFSET&y']=y;
			}else if(elmc=='OPACITY'){
                   var val=this.tokens.shift();
                   properties[elm1]=val;
			}else if(elmc=='OUTLINECOLOR'){
