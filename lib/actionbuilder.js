...
function addTab(val, arrContent, initialval, limitval, contentVal) {
		...
		if (values.type == "textbox") {
			if(values.id.toUpperCase()!="CFORMATSTRING"){
				tr.append('td').append("input").attr("type", "textbox").attr("id", values.id).attr("name", values.id).attr("value", loadVal)
						...
						// .attr("onpaste","ValidatePaste('"+tempid+"')")
	...
	if(ACTIVEPROPERTY.toUpperCase()=="LAYER" || ACTIVEPROPERTY.toUpperCase()=="OUTPUTFORMAT"){
		var strid="";
	...
	function getNodePropsValues(key) {
		...
		if(key == "MetaData") {
			...
			for (var q = 0; q < p.length; q++) {
				...
				if(key.toUpperCase() == "OUTPUTFORMAT"){
					var focount = $('.formatoption').length;
					if(focount>0){
						for (var c = 1; c <= focount; c++) {
							var tempval="";var tempid="formatoption"+c;
				...
				if(key.toUpperCase() == "LAYER"){
					var focount = $('.processing').length;
					if(focount>0){
						for (var c = 1; c <= focount; c++) {
							var tempval="";var tempid="processing"+c;
							if (document.getElementById(tempid)) {
								tempval = document.getElementById(tempid).value;
								if(tempval!=""){result[tempid] = tempval};
				    		}
                 ...
                 for (var k in props) {
                     ...
                     if(ENDVALUES.indexOf(k.toUpperCase()) > -1){
                     	...
                     	if (k.toUpperCase().substring(0, 7) == "PROJTXT") {
                     		DATABUFFER += tempstr + '"' + props[k] + '"\n';var tmp=props[k];console.log(tmp+"___"+tempstr + "\"" + "tmp" + "\"");
                     	}else{
                     		
                     			var key,val;
                     			(k.indexOf(" ")>-1 && k.toUpperCase().substring(0, 6)!="CONFIG")?key='"'+ k + '"':key=k; 
                     			
                     			(props[k].indexOf(" ")>-1 && k.toUpperCase()!="EXPRESSION" && (LOOKUP[k])==null)?val='"'+ props[k] + '"':val=props[k];
                     			//replace integer
