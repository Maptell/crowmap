...
"WEBIMAGEURL","WEBLOG","WEBMAXTEMPLATE","WEBTEMPLATE","WEBTEMPPATH","LAYERNAME","LAYERLABELITEM","LAYERCLASSITEM","LAYERREQUIRES", "STYLESYMBOL"];
...
function addTab(val, arrContent, initialval, limitval, contentVal) {
	...
	for (var i = initialval; i < limitval; i++) {
		...
		if (values.type == "textbox") {
			...
			loadVal = loadVal.replace(/'|#/g,"");
			tr.append('td').append("input").attr("type", "textbox").attr("id", values.id).attr("class", "colorpickr").attr("value", loadVal)
			.attr("readonly","readonly");
			
			if(loadVal){
				
				$("#"+values.id).css('background-color','#'+loadVal);
			}
			...
			$('.colorpickr').colorpicker({
				...
				close: function(event, color){
					...
					$("#"+event.target.id).css('background-color','#'+color.formatted);
				}
				...
				if (id.charAt(0) != "$") {
					...
					if (elm.type == "textbox") {
						...
						if (document.getElementById(id)) {
							...
							if(tmpval!=""){
								val="'#" + tmpval + "'";
								$('#'+id).css('background-color','#'+tmpval);
							}
						}
...
// mandatorycheck();
   ...
   function moveUpDown(element,dir)
   {
   	             var nm=element.name;
	   	 		 ...
	   	 		 if(pos>-1 && ((dir && pos< data.length-1) || (!dir && pos>1)) ){
                  	   	 		 		
	   	 		 		var nxt=getNextElement(pos,type,pname);
	   	 		 		var prv=getPrevElement(pos,type,pname);
	   	 		
	   	 		 		
	   	 		 		dir?data.move(pos,nxt):data.move(pos,prv);
   ...
   function getNextElement(pos,type,pname){
   	   	  	  
   	  for(var j=pos+1;j<data.length;j++)
	  {
	  	 var parname=data[j].parent==null?null:data[j].parent.name;
	   	 if(data[j].type==type && parname==pname) {
    ...
    function getPrevElement(pos,type,pname){
